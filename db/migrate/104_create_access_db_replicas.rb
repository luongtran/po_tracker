...
class CreateAccessDbReplicas < ActiveRecord::Migration
  ...
  def self.up_tables
    ...
    #Lht :create table
    create_table :record_changelogs, {:id => false, :force => true} do |t|
    ...
    #Lht update column
    unless column_exists? :record_changelogs, :record_identifier
      add_column :record_changelogs, :record_identifier, :string
    end
    unless column_exists? :record_changelogs, :action, :string
      add_column :record_changelogs, :action, :string
    end
    unless column_exists? :record_changelogs, :attachment_file_name
      add_column :record_changelogs, :attachment_file_name, :string
    end
    unless column_exists? :record_changelogs, :attachment_content_type
      add_column :record_changelogs, :attachment_content_type, :string
    end
    unless column_exists? :record_changelogs, :attachment_file_size
      add_column :record_changelogs, :attachment_file_size, :integer
    end
    unless column_exists? :record_changelogs, :attachment_updated_at
      add_column :record_changelogs, :attachment_updated_at, :datetime
    end
    
    unless column_exists? :record_changelogs, :modified_by
    ...
    unless column_exists? :piping_class_details, :size_lower
      add_column :piping_class_details, :size_lower, :decimal,:precision => 6, :scale => 3
    end
    unless column_exists? :piping_class_details, :size_upper
  ...
  def self.import
    ...
    table_infos.each do |info|
      ...
      #Lht use LOAD DATA LOCAL INFILE instead of LOAD DATA INFILE
      execute "load data local infile '#{fixtures_location}#{info[:file_name]}' into table po_tracker.#{info[:table_name]} fields terminated by ',' enclosed by '\"' ignore 1 lines;"
