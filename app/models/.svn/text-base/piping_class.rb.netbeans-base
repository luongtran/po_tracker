...
class PipingClass < ActiveRecord::Base
    ...
    #Save all records in cache
    def self.cached_all(order,includes_of=nil)
      criteria = includes_of.nil? ? scoped : includes(includes_of)
      criteria =  criteria.order(order)
      Rails.cache.fetch("#{self.to_s}_all_cache") { criteria.to_a }
    end
  	############################################################################
  	# INSTANCE METHODS
  	############################################################################
