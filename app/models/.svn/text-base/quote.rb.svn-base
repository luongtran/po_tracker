...
class Quote < ActiveRecord::Base
  ...
  def self.filter(params, employee)
	...
	#Find the quote with the previous ID
	def find_previous
		q = Quote.find(:first, :conditions => ['id < ?', self.id], :order => "id DESC", :limit => 1)
		if(q.nil?)
			#go to the last quote if we can't find one before us
			return Quote.last
		end
		return q
	end
	def find_next
		q = Quote.find(:first, :conditions => ['id > ?', self.id], :order => "id ASC", :limit => 1)
		if(q.nil?)
			#go to the first quote if we can't find one after us
			return Quote.first
		end
		return q
	end
