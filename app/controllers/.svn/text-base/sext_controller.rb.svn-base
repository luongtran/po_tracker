class SextController < ApplicationController
  ...
  def index(should_render=true)
    ...
    #Model Caching Adding to PipingClass and Valve
    if klass.eql?(PipingClass) || klass.eql?(Valve)
      records = klass.cached_all(order,Sext.get_joins(klass))
      @records = records
      records = Sext.cached_get_data(records,true, params,"#{klass.to_s}_sext_cache")
      #Implementing search on cached records if search query present
      if params[:query].present?
        search_records = []
        records.each do |record|
          #append record if search query includes any values of a record
          search_records << record if record.values.map(&:to_s).map(&:downcase).any?{|v| v.include?(params[:query].downcase)}
        end
        records = search_records
      #Advanced Search against cached records
      elsif params[:filter].eql?('true')
        search_records = []
        records.each do |record|
          after_filer = []
          #check each value in records at key is including params value if includes then push true to after_filer array
          params.keys.map(&:to_s).each do |key|
            if record.has_key?(key) and params[key] != '' and params[key] != 'No' and params[key] != '[]'
              p_value = params[key]
              a_value = record[key]
              after_filer << a_value.to_s.downcase.include?(p_value.to_s.downcase)
            end
          end
          unless after_filer.uniq.include?(false)
