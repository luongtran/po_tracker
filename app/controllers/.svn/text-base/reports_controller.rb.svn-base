class ReportsController < ApplicationController
  ...
  def sandbox
    ...
    if request.post?
      ...
      cmd = "/usr/local/reportman/printreptopdf #{params[:params]} #{Rails.root}/tmp/#{filename} 2>> #{Rails.root}/tmp/stderr.txt"
      Rails.logger.info cmd
      data = `#{cmd}`
      if data.size > 0
        send_data(data, :type => "application/pdf", :filename => filename + ".pdf")        
      else
        render :text => "<pre>" + IO.read("#{Rails.root}/tmp/stderr.txt") + "</pre>"
      end
    end
  end
