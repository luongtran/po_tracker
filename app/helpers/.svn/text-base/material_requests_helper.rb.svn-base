module MaterialRequestsHelper
  ...
  def previous_quote(quote)
    q = quote.find_previous
    if q.nil?
      return img_button "Previous Quote", "previous.png", ""
    else
    ...
    if(m.nil?)
      ...
      return img_button "Previous Quote", "previous.png", 
                url_for(:action => :quote, 
                  :id => m.id, 
                  :controller => :material_requests, 
                  :vendor_id => q.vendor_id, 
                  :access_key => q.vendor.access_key)
    end
  end

  def next_quote(quote)
    q = quote.find_next
    if q.nil?
      return img_button "Next Quote", "next.png", ""
    else
      m = q.material_request
    end
    if(m.nil?)
