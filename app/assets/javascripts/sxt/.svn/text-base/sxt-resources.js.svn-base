                ...
                listeners: {
                    'rowselect': {
                        fn: function (rowSelectionModel, rowIndex, record) {
                            ...
                            loadValveRelatedRecords(record);
                            // update height to one line if empty.Make box auto-expand height up to 6 lines if populated.
                            updateValveNoteHeightLine();
                ...
                {xtype:'tbspacer'},
                {
                    ...
                    listeners:{
                        'select':function(combo,record,index){
                            ...
                            var store = Ext.getCmp('record_changelogs_grid').store;
                            Ext.apply(store.baseParams,{'piping_class_explicit':true,
...
/* ===========Newly added================================= */
//valve_not textarea: update height to one line if empty.Make box auto-expand height up to 6 lines if populated.
function updateValveNoteHeightLine(){
    var valve_note = $("textarea[name=valve_note]");
    valve_note.css('height', 'auto');
    if(valve_note.val() != ''){
        valve_note.attr('rows', '6');
    }else{
        valve_note.attr('rows', '1');
    }
}
